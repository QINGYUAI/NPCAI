...
| 0.5 | 2026-04-21 | AINPC-Dev | **🟢 M4.2.2 live LLM smoke PASS**（roadmap v0.10）。scene=1 / 2 NPC（均切 ai_config_id=4 通义） / ticks=2 / concurrency=2 全链路真 LLM：tick_log 4/4 success，ai_call_log 12/12（plan=4/speak=4/memory=4），npc_memory 新增 8 行全 `embed_status=embedded`，Qdrant collection `npc_memory` 自动建（status=green / points_count=8 / size=1536 / Cosine）。Y2 指针路径 `resolveEmbedAiConfig → embedText(model=text-embedding-v1) → Qdrant upsert` 完整验证。附加修复：`/api/ai-logs` HTTP 500（mysql2 LIMIT 占位符问题，内联 sanitized 整数）；新增探针 `scripts/debug/probe-embed-dim.ts` + `probe-memory-state.ts`。M4.2.2 整节点收官，进入 M4.2.3。 |
| 0.4 | 2026-04-21 | AINPC-Dev | **✅ M4.2.2.c 前端徽章 + embedding 指针式注入完成**（roadmap v0.9）。后端新增 `engine/memory/embedAiConfig.ts`（60s TTL 缓存的 ai_config 指针 resolver，支持 fallback 到 chat aiCfg 不阻塞 tick）+ `MemoryConfig.embedAiConfigId` / env `MEMORY_EMBED_AI_CONFIG_ID` 开关；`retrieve.ts` / `store.ts` 改走 `resolveEmbedAiConfig()`，embedText 接受 model override，从而解耦「NPC chat provider（如 DeepSeek 无 embedding）」与「embedding provider（如通义千问 `text-embedding-v1`，1536 维，免 Qdrant collection 迁移）」。前端 `types/engine.ts` 扩 `memory_degraded?: boolean`；`Sandbox.vue` 顶栏 WS 之后、Σ 之前插 🧠 徽章，仅降级时显示，`.memory-warn-pill` 复用 meta-warn-flash infinite alternate。`.env.example` 补齐 memory 全量样板 + 中文注释；调试脚本归档 `scripts/debug/probe-ai-config.ts`。测试：后端 67/67 + tsc、前端 vue-tsc + vite build 全绿。**待办**：live smoke 验真实 embedding 入 Qdrant。 |
